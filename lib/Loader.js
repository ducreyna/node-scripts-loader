...
Loader.prototype.load = function (key, script, options) {
	...
	if(options.safe && this._scripts[key] !== undefined) {
		throw new Error("Script already defined");
	}
	...
	if(typeof(script) === 'function') {
		...
		if(!fn) {
			throw new Error("Cannot parse script from file");
		}
	} else {
		throw new Error("Script parameter must be a Function or a String");
	}

	// Add script
	if(this._scripts[key] === undefined) {
		this._scripts[key] = { 'fn': fn };
	}
	...
	if(options.watch && typeof(script) === 'string') {
		...
		this._scripts[key].watcher = fs.watch(script, function (evt) {
			if(evt === 'change') {
				...
				if(!newFn) {
					throw new Error("Cannot reload file: please check it");
				}
